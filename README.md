...
中山大学、广州大学、北京师范大学珠海校区本科教务课表的 Android 客户端：在应用内登录教务，把课表和考试下载到手机，离线查看、手动改课、分享给同学，并支持桌面小组件和上课提醒。

开源地址：<https://github.com/pipidu/sysukcb>

## 功能

- WebView 登录中山大学、广州大学或北师大珠海校区教务，会话只存在本机；重新登录会清掉 WebView 历史和 Cookie
- 在「我的」里切换学校后再登录
- 中山大学、广州大学、北师大珠海校区均可导入课表与考试；「全部导入」覆盖教务当前学期前后各 8 个学期（含已公布的未来课表和考试）。导入成功会覆盖该学期已导入课表，不会叠一份。
- 登录 / 导入成功后回到课表页
- 周视图课表：点顶栏周数选周、左右滑动切周；也可点「全部」或菜单里的学期总课表
- 同一格多节课会在右下角标数量，点开可看全部；卡片显示教师、地点和备注
- 点课程看详情，详情里可直接编辑；三点菜单进入编辑模式后才能点空白加课
- 考试列表：考试周筛选、卡片右侧倒数日（各校同一套界面）
- 导出 / 导入 JSON，方便同学互导；也可从 WakeUp 课程表备份或 CSV 导入
- WebDAV 同步：在「我的 → 同步」填写网盘地址后上传/下载全部学期课表；可填昵称，同一账号下其他昵称的课表出现在「好友」页；可自动同步
- 桌面小组件：今日课程、本周课表、2×2 最近两节课
- 上课与考试通知；主题色可改（预设带名字），显示模式可跟随系统 / 浅色 / 深色；课表卡片用色相拉开的中饱和色、白字，随主题转动色相
...
3. 课表页可点周数、左右滑动或用箭头切换教学周；学年在顶栏菜单里选。考试在底栏「考试」页查看，好友课表在「好友」页。
4. 需要改课时，点开详情里的「编辑」，或打开右上角菜单进入编辑模式后点空白加课。桌面可添加 2×2「最近两节课」或 4×4 本周课表小组件。
5. 换机或备份可在「我的 → 同步」用 WebDAV 上传/下载（坚果云：打开坚果云 → 应用密码 → 粘贴）。填昵称后，同一网盘账号里其他昵称的课表会出现在「好友」页。
6. 也可在「我的」从 WakeUp 课程表导入备份文件或 CSV。
7. 关于与开源地址在「我的」最底部。
